...
?>
<?php // consulta_atenciones.php - Golden refinado (cards), busqueda y paginacion
...
if ($query !== "") {
  ...
  for ($i = 0; $i < 7; $i++) { $params[] = $query; $types .= "s"; }
}
...
$sqlData = "
  SELECT
    id, registro, fecha_hora_atencion, fecha,
    ...
    atencion_en, direccion_servicio, localizacion, ciudad,
    ips_destino, nombre_ips_receptora, nit_ips_receptora,
    ambulancia, conductor, tripulante, medico_tripulante,
...
//          $chips[] = chip($row['servicio'], 'bg-primary');
        ...
        if(!empty($row['ciudad'])) $loc[] = $row['ciudad'];
        ...
        <div class="card card-hover card-compact h-100">
          <div class="card-body lh-tight">
            <div class="d-flex justify-content-between align-items-start mb-compact">
              <div class="lh-tight">
                <div class="fw-semibold mb-compact">Reg. <?php echo h($row['registro']); ?></div>
                <div class="small muted mb-compact">ID <?php echo h($row['id']); ?>  |   <?php echo h($fh); ?></div>
              </div>
    ...
    <?php
      ...
      $rolesAdmin = ['administrativo','master','secretaria'];
