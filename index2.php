...
<div class="row mb-2">
  <div class="col-md-12">
    ...
    <div class="row g-2">
      ...
      <div class="col-auto">
        <button type="button" class="btn btn-primary text-nowrap" onclick="obtenerUbicacion()">Usar mi ubicación</button>
      </div>
    </div>
  </div>
</div>
...
<h3>Adjuntos <span class="hl7-tag">FHIR: DocumentReference</span></h3>  </div>
      ...
      <div class="modal-dialog modal-fullscreen">
        <div class="modal-content">
          ...
          <div class="modal-body d-flex flex-column">
            <div id="modalSignaturePad" class="flex-grow-1" style="border: 1px solid #ccc; border-radius: 8px;">
              <canvas style="width: 100%; height: 100%;"></canvas>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" id="clearModalSignature">Limpiar</button>
            <button type="button" class="btn btn-primary" id="saveModalSignature" data-bs-dismiss="modal">Guardar Firma</button>
          </div>
        </div>
      </div>
    </div>
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const modalPadContainer = document.getElementById('modalSignaturePad');
    const modalCanvas = modalPadContainer.querySelector('canvas');
    const modalSignaturePad = new SignaturePad(modalCanvas, { penColor: '#000' });
    let currentSignatureTarget = null;

    signatureModal.addEventListener('shown.bs.modal', function (event) {
        const button = event.relatedTarget;
        currentSignatureTarget = button.getAttribute('data-sig-target');
        
        // Ajustar tamaño del canvas del modal
    ...
    document.getElementById('saveModalSignature').addEventListener('click', function () {
        if (!modalSignaturePad.isEmpty() && currentSignatureTarget) {
            const dataURL = modalSignaturePad.toDataURL();
            firmas[currentSignatureTarget].fromDataURL(dataURL);
            updateSignatureData(currentSignatureTarget);
        }
    });
    const firmas = {
      paramedico: new SignaturePad(document.querySelector('#firmaParamedico canvas'), {
        penColor: '#34446d',
        minWidth: 0.5, // Grosor mínimo del trazo
        maxWidth: 1.5  // Grosor máximo del trazo
      }),
      medico: new SignaturePad(document.querySelector('#firmaMedico canvas'), {
        penColor: '#a25505',
        minWidth: 0.5, maxWidth: 1.5
      }),
      paciente: new SignaturePad(document.querySelector('#firmaPaciente canvas'), {
        penColor: '#000',
        minWidth: 0.5, maxWidth: 1.5
      }),
      medicoReceptor: new SignaturePad(document.querySelector('#firmaMedicoReceptor canvas'), {
        penColor: '#0e6b0e',
        minWidth: 0.5, maxWidth: 1.5
      }),
      desistimiento: new SignaturePad(document.querySelector('#firmaDesistimiento canvas'), {
        penColor: '#d32f2f',
        minWidth: 0.5, maxWidth: 1.5
      })
    };
    ...
    Object.keys(firmas).forEach(tipo => {
      firmas[tipo].addEventListener("endStroke", () => updateSignatureData(tipo));
    });
    ...
    // --- LÓGICA DE BOTONES ACEPTAR/RECHAZAR ---
    const consentimientoContainer = document.getElementById('consentimiento-container');
    const desistimientoContainer = document.getElementById('desistimiento-container');
    const medicoReceptorFields = [
        document.getElementById('nombre_medico_receptor'),
        document.getElementById('tipo_id_medico_receptor'),
        document.getElementById('id_medico_receptor')
        // El campo de la firma se validará por separado
    ];

    document.getElementById('btn-aceptar-atencion').addEventListener('click', function() {
        consentimientoContainer.style.display = 'block';
        desistimientoContainer.style.display = 'none';
        firmas.desistimiento.clear();
        updateSignatureData('desistimiento');
        // Hacer los campos del médico receptor obligatorios
        medicoReceptorFields.forEach(field => field.required = true);
    });

    document.getElementById('btn-rechazar-atencion').addEventListener('click', function() {
        consentimientoContainer.style.display = 'none';
        desistimientoContainer.style.display = 'block';
        firmas.paciente.clear();
        updateSignatureData('paciente'); // Limpiar datos de firma de paciente
        // Hacer los campos del médico receptor opcionales
        medicoReceptorFields.forEach(field => {
            // Para los inputs de texto y select, se usa 'required'
            if (field.tagName === 'INPUT' || field.tagName === 'SELECT') {
                field.required = false;
                // Opcional: Limpiar los campos si se rechaza
                // field.value = '';
            }
        });
    });
    ...
    const medicamentosContainer = document.getElementById('medicamentos-aplicados-container');
    const motivoTrasladoContainer = document.getElementById('motivo-traslado-container');
    const downtonContainer = document.getElementById('downton-container');
    ...
    function toggleMedicoTripulante() {
        ...
        medicamentosContainer.style.display = esTAM ? 'block' : 'none';

        downtonContainer.style.display = esTAM ? 'block' : 'none';
        if (!esTAM) {
        ...
        // Lógica para el diagnóstico
        if (esTAM) {
            diagnosticoContainer.style.display = 'block';
            motivoTrasladoContainer.classList.remove('col-md-12');
            motivoTrasladoContainer.classList.add('col-md-6');
        } else {
            diagnosticoContainer.style.display = 'none';
            $('#diagnostico_principal').val(null).trigger('change'); // Limpiar Select2
            motivoTrasladoContainer.classList.remove('col-md-6');
            motivoTrasladoContainer.classList.add('col-md-12');
        }
    }
    servicioField.addEventListener('change', toggleMedicoTripulante);
    toggleMedicoTripulante(); // Llamada inicial

    // --- LÓGICA MEDICAMENTOS APLICADOS ---
    const listaMedicamentos = document.getElementById('lista-medicamentos');
    const templateMedicamento = document.getElementById('medicamento-template');

    document.getElementById('btn-agregar-medicamento').addEventListener('click', function() {
      const clone = templateMedicamento.content.cloneNode(true);
      // Inicializar flatpickr en el nuevo campo de hora
      flatpickr(clone.querySelector('.medicamento-hora-input'), timeConfig);
      listaMedicamentos.appendChild(clone);
    });

    listaMedicamentos.addEventListener('click', function(e) {
      if (e.target && e.target.classList.contains('btn-remover-medicamento')) {
        e.target.closest('.medicamento-item').remove();
      }
    });

    // --- LÓGICA ESCALA DOWNTON ---
    const downtonChecks = document.querySelectorAll('.downton-check');
    const downtonResultado = document.getElementById('downton_resultado');
    const downtonRiesgo = document.getElementById('downton_riesgo');
    const downtonTotalInput = document.getElementById('downton_total_hidden');

    function calcularDownton() {
      let total = 0;
      // Sumar puntos de Caídas, Déficit, Estado Mental y Deambulación
      total += parseInt(document.querySelector('input[name="downton_caidas"]:checked')?.value || '0');
      total += parseInt(document.querySelector('input[name="downton_deficit"]:checked')?.value || '0');
      total += parseInt(document.querySelector('input[name="downton_mental"]:checked')?.value || '0');
      total += parseInt(document.querySelector('input[name="downton_deambulacion"]:checked')?.value || '0');

      // Para medicamentos, se suma 1 punto si se ha seleccionado CUALQUIER medicamento que no sea "Ninguno".
      if (document.querySelector('input[name="downton_medicamentos"]:checked')?.value !== "0") {
          total += 1;
      }

      downtonTotalInput.value = total;
      downtonResultado.value = total + " puntos";
      if (total >= 2) {
        downtonRiesgo.textContent = "Riesgo Alto de Caídas";
        downtonRiesgo.style.color = 'red';
      } else {
        downtonRiesgo.textContent = "Riesgo Bajo de Caídas";
        downtonRiesgo.style.color = 'green';
      }
    }
    downtonChecks.forEach(input => input.addEventListener('change', calcularDownton));
    calcularDownton(); // Cálculo inicial
...
function obtenerUbicacion() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function(position) {
      ...
      document.getElementById('direccion_servicio').value = `Lat: ${lat.toFixed(6)}, Lon: ${lon.toFixed(6)}`;
