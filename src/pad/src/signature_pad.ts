/**
 * Signature Pad - A canvas based signature pad.
 * @author Thomas J Bradley <hey@thomasjbradley.ca>
 * @author Szymon Nowak <szimek@gmail.com>
 * @license MIT
...
export default class SignaturePad extends SignatureEventTarget {
  ...
  private _touchEventToSignatureEvent(event: TouchEvent): SignatureEvent | null {
  ...
  private _handleMouseDown = (event: MouseEvent): void => {
    ...
    const signatureEvent = this._pointerEventToSignatureEvent(event);
    if (!signatureEvent) {
      return;
    }
    this._strokeBegin(signatureEvent);
  };
  ...
  private _handleMouseMove = (event: MouseEvent): void => {
    ...
    this._strokeMoveUpdate(signatureEvent);
  };
  ...
  private _handleMouseUp = (event: MouseEvent): void => {
    ...
    this._strokeEnd(signatureEvent);
  };
  ...
  private _handleTouchStart = (event: TouchEvent): void => {
    ...
    const signatureEvent = this._touchEventToSignatureEvent(event);
    if (!signatureEvent) {
      return;
    }
    this._strokeBegin(signatureEvent);
  ...
  private _handleTouchMove = (event: TouchEvent): void => {
    ...
    if (!this._drawingStroke) {
      ...
      this._strokeEnd(signatureEvent, false);
      return;
    }
