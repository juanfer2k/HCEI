...
// Incluir el script de control de acceso
...
function forbid_admin_touching_master(mysqli $conn, int $targetUserId): bool {

    // true => PROHIBIR si actor es Administrativo y el objetivo es Master

    if (($_SESSION['usuario_rol'] ?? '') !== 'Administrativo') return false;

    $st = $conn->prepare("SELECT rol FROM tripulacion WHERE id = ?");
